...
- keep_dem_raster (bool): If True, the individual DEM .tif files will be kept after processing.

Returns:
flow_lenght (dataframe): A DataFrame containing the maximum flow length for each basin.
...
for index, basin in tqdm(basin_shapefile.iterrows(), 
    ...
    dem_files = [
        os.path.join('../temporary_working_dir', f)
        for f in os.listdir('../temporary_working_dir') if f.endswith('.tif')
    ]
    ...
    merged_clipped_dem = f'../temporary_working_dir/dem_{basin_id}.tif'
    ...
    plt.title(f'DEM for Basin {basin_id}')
    plt.xlabel('Longitude', fontsize=12)
    plt.ylabel('Latitude', fontsize=12)
    plt.tight_layout()
    plt.savefig(f'../figures/dem_{basin_id}_clipped.png')
    #plt.show()
    plt.close()
    ...
    with rasterio.open(merged_clipped_dem) as src:
        ...
        projected_dem_file = f'../temporary_working_dir/dem_{basin_id}_clipped_projected.tif'
    ...
    with rasterio.open(f'../temporary_working_dir/flow_length_{basin_id}.tif') as src:
        ...
        plt.ylabel('Latitude', fontsize=12)
        plt.tight_layout()
        plt.savefig(f'../figures/flow_length_{basin_id}.png')
        #plt.show()
        plt.close()
    ...
    # If keep_dem_raster is False, remove the individual DEM and flow length files
    if not keep_dem_raster:
        os.remove(merged_clipped_dem)
        os.remove(f'../temporary_working_dir/dem_{basin_id}_clipped_projected.tif')
        os.remove(f'../temporary_working_dir/dem_{basin_id}_filled.tif')
        os.remove(f'../temporary_working_dir/flow_length_{basin_id}.tif')
